...
structure TreeRenderOpts where
  ...
  /-- When `false` (default), auto-generated declarations
      (recursors, casesOn, match helpers, etc.) are collapsed
      to `[N auto-generated]`. When `true`, they are shown
      like user-written declarations. -/
  expandAutoGenerated : Bool := false
...
/-- Sort and deduplicate the deps map for deterministic output.
    ...
    (e.g., via both `exprRefType` and `recParent`), prefer the
    more specific structural reason over generic expr refs. -/
private def sortDepsMap (graph : TcbGraphResult)
  ...
  for (parent, arr) in graph.depsMap do
    ...
    for (child, reason) in arr do
      ...
      | some prev =>
        -- Prefer structural over generic expr refs,
        -- and exprRefType over exprRefBody
        match prev, reason with
        | .exprRefBody, .exprRefBody => pure ()
        | .exprRefBody, _ =>
          reasonMap := reasonMap.insert child reason
        | .exprRefType, .exprRefType => pure ()
        | .exprRefType, .exprRefBody => pure ()
        | .exprRefType, _ =>
          reasonMap := reasonMap.insert child reason
        | _, _ => pure ()
...
private partial def renderNode
  ...
  -- Separate auto-generated from user-written children
  let mut userChildren :
      Array (Name × DepReason) := #[]
  let mut autoGenCount : Nat := 0
  if opts.expandAutoGenerated then
    userChildren := projectChildren
  else
    for (child, r) in projectChildren do
      if isAutoGenerated child then
        autoGenCount := autoGenCount + 1
      else
        userChildren := userChildren.push (child, r)

  let withAutoGen :=
    if !opts.expandAutoGenerated && autoGenCount > 0 then
      userChildren.push
        (`_leanTcb.autoGenPlaceholder, .exprRefType)
    else
      userChildren
  ...
  let allChildren :=
    if !opts.expandLibrary && libCount > 0 then
      withAutoGen.push
        (`_leanTcb.libPlaceholder, .exprRefType)
    else
      withAutoGen
  ...
  for i in [:allChildren.size] do
    ...
    if child == `_leanTcb.autoGenPlaceholder then
      let childConnector :=
        if childIsLast then "└── " else "├── "
      let agLine :=
        s!"{childIndent}{childConnector}\
          [{autoGenCount} auto-generated]"
      lines := lines.push agLine
    else if child == `_leanTcb.libPlaceholder then
      let childConnector :=
        if childIsLast then "└── " else "├── "
      let depWord :=
        if libCount == 1 then "dependency"
        else "dependencies"
      let libLine :=
        s!"{childIndent}{childConnector}\
...
def renderTree (env : Environment) (graph : TcbGraphResult)
  ...
  -- Discoverability tip
  allLines := allLines.push
    "Tip: #tcb_why <entry> <name> to trace \
      why a name is included"
  allLines := allLines.push ""
