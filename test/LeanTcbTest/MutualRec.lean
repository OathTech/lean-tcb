...
elab "#test_tree_recursor_parent_reason" : command => do
  ...
  | .ok graph =>
    -- Use expandAutoGenerated to see Color.rec (auto-gen)
    let opts : TreeRenderOpts :=
      { expandAutoGenerated := true }
    let output := renderTree env graph opts
    -- Color.rec should appear in the expanded tree
    ...
    -- now show "parent inductive (recursor)" under Color.rec
    -- instead of the generic "referenced in type"
    unless (output.splitOn "parent inductive (recursor)").length
        > 1 do
      throwError s!"expected 'parent inductive (recursor)' \
        in tree: {output}"
...
elab "#test_graph_recparent_in_depsmap" : command => do
  ...
  -- discovered via exprRefType first).
  match computeTcbGraph env #[`colorToNat] with
  | .ok graph =>
