...
-- ═══════════════════════════════════════════════
-- Auto-generated collapsing test
-- ═══════════════════════════════════════════════

-- Fixture: inductive + pattern-matching def to generate
-- auto-gen intermediaries (match_1, casesOn, rec, etc.)
inductive TreeColor where | r | g | b

def treeColorToNat : TreeColor → Nat
  | .r => 0 | .g => 1 | .b => 2

elab "#test_tree_auto_gen_collapse" : command => do
  let env ← getEnv
  match computeTcbGraph env #[`treeColorToNat] with
  | .ok graph =>
    -- Default (collapsed) should show auto-generated count
    let collapsed := renderTree env graph
    unless (collapsed.splitOn "auto-generated").length
        > 1 do
      throwError
        s!"expected '[N auto-generated]' in \
          collapsed tree: {collapsed}"
    -- Expanded should show the actual auto-gen names
    let expanded := renderTree env graph
      { expandAutoGenerated := true }
    if (expanded.splitOn "auto-generated").length
        > 1 then
      throwError
        s!"expanded tree should not contain \
          'auto-generated' placeholder: {expanded}"
    -- Expanded should have more lines than collapsed
    let collapsedLines :=
      (collapsed.splitOn "\n").length
    let expandedLines :=
    ...
    unless expandedLines > collapsedLines do
      throwError
        s!"expanded ({expandedLines} lines) should \
          have more lines than collapsed \
          ({collapsedLines})"
    logInfo "✓ tree auto-gen collapse: PASS"
  | .error msg => throwError msg
